...
from sklearn.preprocessing import LabelEncoder  # Ensure scikit-learn is imported
...
location_names = [
    "Adelaide", "Albany", "Albury", "AliceSprings", "BadgerysCreek", "Ballarat", "Bendigo", "Brisbane",
    "Cairns", "Canberra", "Cobar", "CoffsHarbour", "Dartmoor", "Darwin", "GoldCoast", "Hobart", "Katherine",
    "Launceston", "Melbourne", "MelbourneAirport", "Mildura", "Moree", "MountGambier", "MountGinini", "Newcastle",
    "Nhil", "NorahHead", "NorfolkIsland", "Nuriootpa", "PearceRAAF", "Penrith", "Perth", "PerthAirport",
    "Portland", "Richmond", "Sale", "SalmonGums", "Sydney", "SydneyAirport", "Townsville", "Tuggeranong",
...
# Display tables inside expander
...
# App title
st.title("🌧 Rain Tomorrow Prediction App (AUSTRALIA)")
...
# Categorical inputs
location_name = st.sidebar.selectbox("📍 Location", list(label_encoder_location.classes_))
Location = label_encoder_location.transform([location_name])[0]
st.sidebar.markdown(f"**Selected Location Encoded As:** `{Location}`")

gust_name = st.sidebar.selectbox("🌬️ Wind Gust Direction", list(label_encoder_windgust.classes_))
WindGustDir = label_encoder_windgust.transform([gust_name])[0]
st.sidebar.markdown(f"**Encoded Gust Direction:** `{WindGustDir}`")

wind9am_name = st.sidebar.selectbox("🧭 Wind Direction at 9AM", list(label_encoder_wind9am.classes_))
WindDir9am = label_encoder_wind9am.transform([wind9am_name])[0]
st.sidebar.markdown(f"**Encoded WindDir9am:** `{WindDir9am}`")

wind3pm_name = st.sidebar.selectbox("🧭 Wind Direction at 3PM", list(label_encoder_wind3pm.classes_))
...
# Input dataframe
input_data = pd.DataFrame([[MinTemp, Rainfall, Evaporation, Sunshine, WindGustSpeed,
                            WindSpeed9am, WindSpeed3pm, Humidity9am, Humidity3pm,
                            Pressure9am, Cloud9am, Cloud3pm, Location, WindGustDir,
                            WindDir9am, WindDir3pm, RainToday, year, month, day]],
                          columns=["MinTemp", "Rainfall", "Evaporation", "Sunshine", "WindGustSpeed",
                                   "WindSpeed9am", "WindSpeed3pm", "Humidity9am", "Humidity3pm",
                                   "Pressure9am", "Cloud9am", "Cloud3pm", "Location", "WindGustDir",
                                   "WindDir9am", "WindDir3pm", "RainToday", "year", "month", "day"])
